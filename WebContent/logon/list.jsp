...
%>
	...
	<td align="center" width="50"><%=sdf.format(article.getReg_date()) %></td>
	<td align="center" width="100"><%=article.getPasswd() %></td>
	<td align="center" width="100">
	<a href="modifyForm.jsp?id=<%=article.getId() %>">수정</a>
	<a href="deletePro.jsp?id=<%=article.getId() %>"></a><a href ="deletePro.jsp?id=<%=article.getId()%>&passwd=<%=article.getPasswd() %>">삭제</a>
	</td>
	<%} %>
</table>
...
<%	
	if(count>0){
		...
		<a href="list.jsp?page=<%=startPage-5 %>">[이 전]</a>
<%		}		
